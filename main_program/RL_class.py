...
class ASVEnv(Env):
    ...
    def __init__(self):
        ...
        self.observation_space = Box(low=np.array(
            [0, 0]), high=np.array([20, 1]), dtype=np.float64)
        ...
        self.boat.phat = np.array([[self.boat.x + destination_distance * np.cos(
            self.boat.theta)], [self.boat.y + destination_distance * np.sin(self.boat.theta)]])
        # Set the distance to the objective and the risk of collision
        self.state = np.array([dist(np.array([[self.boat.x], [
                              self.boat.y]]), self.boat.phat), self.boat.collision_risk], dtype=np.float64)
    ...
    def reset(self):
        ...
        # Set the distance to the objective and the risk of collision
        self.state = np.array(
            [dist(np.array([[self.boat.x], [self.boat.y]]), self.boat.phat), 0], dtype=np.float64)
