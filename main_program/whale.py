...
class Whale(SeaObject):
    ...
    def draw(self, ax, Ɛ, col='blue', coef=0.1, w=2):
        ...
        ax.plot(M_transformed[0], M_transformed[1],
                color=col, linewidth=w, zorder=1)
        ax.add_patch(
            Polygon(M_transformed[:2].T, facecolor=col, edgecolor=None, zorder=0))
        """ Display of the zones """
        draw_circle(ax, self.x, self.y, self.r,
                    'red')                           # DCPA zone to avoid related to the boat
        # DCPA zone extended for safety : manoeuvering area
