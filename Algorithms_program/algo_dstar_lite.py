...
class DStarLite:
    ...
    def initialize(self, start: Node, goal: Node):
        '''x_offset = -min(self.x_min_world, 0)
        y_offset = -min(self.y_min_world, 0)
        self.start.x = start.x + x_offset
        self.start.y = start.y + y_offset
        self.goal.x = goal.x + x_offset
        self.goal.y = goal.y + y_offset'''
        ...
        '''
        print("goal.x",goal.x)
        print("self.x_min_world",self.x_min_world)
    ...
    def compute_current_path(self):
        ...
        #print("current point is ", current_point)           #to get the adress
        #print("coord", current_point.x, current_point.y)    #to get the coordinate
        ...
        # print("the path is ", path)   #print path adress
        return path
    ...
    def dstarl(self, start: Node, goal: Node,
        ...
        pathy = []
        path=[]
        # print("start et goal",start,goal)
        ...
        #print("pathx ",(pathx[-1],pathy[-1]))
        path.append((pathx[-1],pathy[-1]))
                ...
                if show_animation:
                    ...
                    if not self.compare_paths(current_path, new_path):
                        ...
                        current_path_image = self.display_path(current_path,
                                                               ".c")
                        plt.pause(pause_time)
        #print("Path found in", outer_iterations)
        return path
...
def main():
    ...
    path=dstarlite.dstarl(Node(x=sx, y=sy), Node(x=gx, y=gy))
